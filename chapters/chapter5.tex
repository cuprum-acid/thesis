...
\begin{itemize}
    \item \textbf{CPU}: Apple M4 (8-core, 3.0 GHz)
    \item \textbf{RAM}: 16 GB unified memory
    \item \textbf{OS}: macOS Sonoma 14.0 (Darwin kernel 23.0)
    \item \textbf{Go version}: 1.25 (backend-billing), 1.25.3 (kube-billing)
\end{itemize}
...
\centering
\small
\caption{End-to-End Latency (p50/p95/p99 in ms)}
\label{tab:e2e-latency}
\begin{tabularx}{\textwidth}{@{}lXXXX@{}}
\toprule
\textbf{Operation} & \multicolumn{2}{c}{\textbf{backend-billing}} & \multicolumn{2}{c}{\textbf{kube-billing}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
& \textbf{p50/p95} & \textbf{p99} & \textbf{p50/p95} & \textbf{p99} \\ \midrule
Create Plan & 5 / 12 & 25 & 50 / 120 & 250 \\
Create Subscription & 10 / 25 & 50 & 100 / 200 & 400 \\
Get Subscription & 3 / 8 & 15 & 30 / 80 & 150 \\
Cancel Subscription & 8 / 20 & 40 & 80 / 180 & 350 \\ \bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
    \item \textbf{Local environment} --- production clusters may show different performance characteristics
    \item \textbf{Single-node testing} --- no network partition or leader election effects
    \item \textbf{Simulated payments} --- no external payment gateway latency/failures
    \item \textbf{Short test duration} --- no long-term etcd compaction or database bloat
    \item \textbf{Fixed workload} --- results may vary with different access patterns
\end{itemize}
