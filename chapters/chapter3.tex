...
\textbf{Status subresource} (managed by controller):
\begin{itemize}
    \item \texttt{status.state} (string): Enum (Active, PaymentError, Error)
    ...
    \item \texttt{status.observedGeneration} (integer): Tracks status freshness against spec
    \item \texttt{status.conditions} (array): Standard conditions (\texttt{Active}, \texttt{BillingPlanNotFound}, \texttt{PaymentError})
\end{itemize}
...
Both systems were deployed and tested on identical hardware:
\begin{itemize}
    \item \textbf{CPU}: Apple M4 (single-core performance isolated for benchmarks)
